...
function MessageAvatar({ role, isSearch }: { role: "user" | "assistant"; isSearch?: boolean }) {
  return (
    <div
      className={cn(
    ...
    >
      {role === "user" ? <User className="h-4 w-4" /> : isSearch ? <Search className="h-4 w-4" /> : <Bot className="h-4 w-4" />}
    </div>
  );
}
