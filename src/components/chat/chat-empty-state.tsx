...
export function ChatEmptyState({ mode }: ChatEmptyStateProps) {
  const isSemanticSearch = mode === "search";
      ...
      <div className="max-w-lg w-full space-y-6">
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-semibold tracking-tight">
            {mode === "chat" ? "Chat with your Library" : isSemanticSearch ? "Search your Documents" : "Full-Text Search"}
          </h2>
          <p className="text-muted-foreground text-sm leading-relaxed max-w-sm mx-auto">
            {mode === "chat"
              ? "Ask questions about your documents and get answers with sources. Use filters to narrow your search."
              : isSemanticSearch
                ? "Find conceptually similar passages and related information across your library."
                : "Find exact words and phrases across all of your documents instantly."}
          </p>
        </div>
      </div>
    </div>
