...
export function SourcePreviewDialog({ source, open, onOpenChange }: SourcePreviewDialogProps) {
  ...
  const { data: pageRecord } = usePageByNumber(open && uploadId ? uploadId : "", open && pageNumber != null ? pageNumber : 0);
  const pageId = pageRecord?.id ?? null;

  const { data: markdown, isLoading: isLoadingMarkdown } = usePageMarkdown(open ? (pageId ?? undefined) : undefined);
        ...
        <div className="flex-1 min-h-0 border rounded-lg bg-card flex flex-col overflow-hidden">
          ...
          <div className="flex-1 min-h-0 overflow-auto p-4">
            {isLoadingMarkdown || (open && uploadId && pageNumber != null && !pageId) ? (
              ...
              <div className="text-sm text-foreground/80 font-serif leading-relaxed whitespace-pre-wrap">{renderedContent}</div>
            ) : (
